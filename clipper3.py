...
class SimulationState:
    def __init__(self, params: Dict[str, Any], time_step: float, clock: float):
        self.params = params
        if self.params.plt_metrics is not None or self.params.data_metrics is not None:
            self.timesteps_save = []
            self.clipper_europa_dist_save = []
            self.clipper_vel_save = []
            self.clipper_acc_save = []
...
class Simulation:
    ...
    def timer_cb(self, obj, value):
        ...
        if self.state.params.plt_metrics is not None or self.state.params.data_metrics is not None:
            self.state.timesteps_save.append(clock)
            self.state.clipper_europa_dist_save.append(dClipper2Body)
            self.state.clipper_vel_save.append(ClipperVelocity)
            self.state.clipper_acc_save.append(ClipperAcceleration)
...
class MainWindow(QMainWindow):
    ...
    def quit_cb(self):
        ...
        if self.state.params.data_metrics is not None:
            self.save_metrics_data(self.state.params.data_metrics)
        sys.exit(0)
    ...
    def save_metrics_plts(self, plt_metrics_path):
        tsteps = self.state.timesteps_save

        plt.figure()
        plt.plot(tsteps, self.state.clipper_europa_dist_save, color=(1.0, 1.0, 0))
        ...
        plt.figure()
        plt.plot(tsteps, self.state.clipper_vel_save, color=(0.016, 0.85, 1.0))
        plt.xlabel("clock time (s)")
        ...
        plt.figure()
        plt.plot(tsteps, self.state.clipper_acc_save, color=(1, 0.063, 0.96))
        ...
        l1, = ax1.plot(tsteps, self.state.clipper_acc_save, color=(1, 0.063, 0.96))
        ...
        ax2 = ax1.twinx()
        l2, = ax2.plot(tsteps, self.state.clipper_europa_dist_save, color=(1.0, 1.0, 0))
        ax2.set_yticks([])
        ax3 = ax1.twinx()
        l3, = ax3.plot(tsteps, self.state.clipper_vel_save, color=(0.016, 0.85, 1.0))
        ax3.set_yticks([])
    ...
    def save_metrics_data(self, data_metrics_path):
        metric_df = pd.DataFrame()
        metric_df['timestep'] = self.state.timesteps_save
        metric_df['Clipper-Europa distance'] = self.state.clipper_europa_dist_save
        metric_df['velocity'] = self.state.clipper_vel_save
        metric_df['acceleration'] = self.state.clipper_acc_save
        metric_df.to_csv(os.path.join(data_metrics_path, "clipper_metrics.csv"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_metrics', type=str, nargs='?', const='', help='directory to save metrics csv from simulation run')

...
class Units:
    ...
    def interval(d1: date, d2: date):
        return d2.time()-d1.time()

    def duration(seconds=0, minutes=0, hours=0, days=0, weeks=0, months=0, years=0, output='seconds'):
    ...
    def to_unit_rgb(rgb: np.ndarray):
        r = float(rgb[0]/255.)
        g = float(rgb[1]/255.)
        b = float(rgb[2]/255.)
        return [r, g, b]
...
class DynamicFrame(Frame):
    ...
    def get_frame(self, et: float) -> Frame:
        ...
        Y = origin - self.object.get_position(et+5)
...
class ClipperBodyDynamicFrame(DynamicFrame):
    ...
    def get_frame(self, et: float) -> Dict[str, np.ndarray]:
        ...
        Y /= Ynorm
        X = self.reference.get_position(et) - targetp
        X -= np.dot(X, Y)
        X /= np.linalg.norm(X)
        Z = np.cross(X, Y)
...
class MainWindow(QMainWindow):
    ...
    def camera_change_cb(self, caller, event):
        ...
        if self.state.params.paused or event=='MouseWheelBackwardEvent' or event=='MouseWheelForwardEvent':
            ...
            self.state.current_camera[frame_name] = global2local(frame=self.data.dynbodyframe[frame_name], globalcam=self.graphics.renderers['bodies'].GetActiveCamera(), et=self.state.clock)
            self.state.current_camera[frame_name].SetFocalPoint(0, 0, 0)
    ...
    def date_change_cb(self, datetime: QDateTime):
        ...
        renderer.RemoveActor(
            self.graphics.all_actors['Clipper']['Clipper forward orbit'])
        # renderer.RemoveActor(
